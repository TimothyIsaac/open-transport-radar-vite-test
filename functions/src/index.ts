...
app.post("/stations", stationsRequestValidation, async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        const results = await client.nearby({ type: "location", latitude, longitude }, { distance })
...
const departureRequestValidation = [
    query('stationID').isNumeric(),

]
app.get("/departures", departureRequestValidation, async (req: express.Request, res: express.Response) => {
    try {
        const errors = validationResult(req)
        if (!errors.isEmpty()) {
            return res.status(400).json(errors.array())
...
app.get("/radar", radarRequestValidation, async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        functions.logger.info(req.body)
        const { north, west, south, east, duration = 10, frames = 6, results = 10 } = req.body
        ...
        const radar_results = await client.radar({ north, west, south, east }, { duration, frames, results })
